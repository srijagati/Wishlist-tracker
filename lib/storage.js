...
const UNSEEN_DROPS_KEY = "unseenDropIds";
...
export async function addUnseenDrop(id) {
  const data = await chrome.storage.local.get(UNSEEN_DROPS_KEY);
  const ids = new Set(data[UNSEEN_DROPS_KEY] || []);
  ids.add(id);
  await chrome.storage.local.set({ [UNSEEN_DROPS_KEY]: [...ids] });
}

export async function getUnseenDropIds() {
  const data = await chrome.storage.local.get(UNSEEN_DROPS_KEY);
  return data[UNSEEN_DROPS_KEY] || [];
}

export async function clearUnseenDrops() {
  await chrome.storage.local.set({ [UNSEEN_DROPS_KEY]: [] });
}
